...
class DynamicNet(nn.Module):
    ...
    def get_graph(self):
        ...
        def process_block(configs, start_node_idx):
            ...
            for cfg in configs:
                ...
                # Initialisation de la liste de voisins pour le noeud précédent
                ...
                if isinstance(cfg, ResBlockCfg):
                    ...
                    # 2. Encodage du Parent (avec le nombre d'enfants dans index 8)
                    ...
                    # 4. Connexions du bloc
                    # Entrée -> Container
                    local_adj[current_prev_node].append(this_node_idx)
                    
                    # Container -> Sortie (Skip Connection)
    ...
    def save_model(self, filename):
        ...
        # Pour sauvegarder sur disque, on convertit le dict en listes (COO)
        # car np.savez ne gère pas bien les dictionnaires Python.
        # Mais le code 'live' utilise le dictionnaire.
        _, edge_index = DynamicNet.to_gnn_format(features_x, adj_dict)

        np.savez_compressed(
            filename, 
            adj=edge_index.numpy(), # On sauvegarde sous forme matricielle pour compatibilité
            features=features_x,
            weights=weights_w            
        )
        ...
        def recursive_parser(rows_queue):
            ...
            while len(rows_queue) > 0:
                ...
                if type_idx == 6: # ResBlock
                    rows_queue.pop(0) # Consomme le header du bloc
                    count_children = int(vec[8])
                    use_proj = (vec[14] > 0.5)
                    
                    sub_layers = []
                    # On sait qu'on doit récupérer 'count_children' éléments de haut niveau
                    # Attention: un enfant peut être lui-même un bloc complexe.
                    # Il faut donc appeler le parser 'count_children' fois.
                    for _ in range(count_children):
                        # Cette logique suppose que recursive_parser retourne une liste,
                        # on prend le premier élément de cette liste comme étant l'enfant direct
                        # Mais recursive_parser consomme toute la queue...
                        # CORRECTION LOGIQUE:
                        # On ne peut pas juste appeler recursive_parser car il va tout manger.
                        # On doit extraire UN objet complet.
    ...
    def load_model(filename):
        ...
        X = data['features']
        W = data['weights']  
        # On ignore 'adj' lors du chargement, X suffit pour reconstruire
